...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("chunked benchmark: {AUDIO_SECONDS}s dummy audio, {ITERATIONS} iterations");

    let mut total_seconds = 0.0;
    for iteration in 1..=ITERATIONS {
        let started = Instant::now();
        let probabilities = forward_audio_in_chunks(&mut vad, &audio)?;
...
fn forward_audio_in_chunks(vad: &mut SileroVad, audio: &[f32]) -> Result<Vec<f32>, Box<dyn Error>> {
    vad.reset();
    let mut probabilities = Vec::new();

    for chunk in audio.chunks(DEFAULT_CHUNK_SAMPLES) {
        let mut padded_chunk = [0.0_f32; DEFAULT_CHUNK_SAMPLES];
        padded_chunk[..chunk.len()].copy_from_slice(chunk);
        probabilities.push(vad.forward_chunk(&padded_chunk)?);
    }
